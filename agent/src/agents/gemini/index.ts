...
export function query({
  ...
  if (mcpServers && Object.keys(mcpServers).length > 0) {
    console.warn('mcpServers configuration is not directly supported by Gemini CLI command line');
  ...
  if (typeof prompt === 'string') {
    ...
    throw new Error('AsyncIterable prompts are not supported yet. Please use string prompts.');
  }

  // Gemini CLIの実行可能ファイルの確認
  if (!existsSync(pathToGeminiExecutable) && pathToGeminiExecutable === 'gemini') {
    // グローバルコマンドとして実行を試行
  }

  console.debug(`Spawning Gemini CLI process: ${pathToGeminiExecutable} ${args.join(' ')}`);
  ...
  const processExitPromise = new Promise<void>((resolve, reject) => {
    ...
    child.on('error', (error) => {
      ...
      if (abortController.signal.aborted) {
        ...
        reject(new Error(`Failed to spawn Gemini CLI process: ${error.message}`));
      }
    });
  });
  ...
  child.on('error', (error) => {
    if (abortController.signal.aborted) {
      ...
      geminiQuery.setError(new Error(`Failed to spawn Gemini CLI process: ${error.message}`));
    }
  });
