...
const ENCRYPTION_KEY =
  process.env.GIT_AUTH_ENCRYPTION_KEY || 'miniapp-git-auth-encryption-key-2024'
...
export async function createMiniAppGitAuth(
  ...
  try {
    ...
    const { error } = await supabase.from('miniapp_git_auth').insert({
...
export async function checkMiniAppGitAuthExists(
  ...
  try {
    ...
    console.error(
      `❌ Error checking Git auth existence for ${miniAppId}:`,
      error
    )
    return false
  }
